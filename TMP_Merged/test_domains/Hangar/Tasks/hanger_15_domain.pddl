...
(define (domain hanger)
  ...
  (:action move
    ...
    :effect (and
        ...
        (when (= ?finalLoc left_wing)
            (probabilistic
                0.85 (and (at ?a ?finalLoc) (not (at ?a ?startLoc)))
                0.15 (at ?a about_left_wing)))
        (when (= ?finalLoc right_wing)
            (probabilistic
                0.85 (and (at ?a ?finalLoc) (not (at ?a ?startLoc)))
                0.15 (at ?a about_right_wing)))
  ...
  (:action inspect_part2
    ...
    :effect (and
        ...
        (when (status ?loc fault_exists) ( probabilistic 0.85 ( fault_detected ?loc)))
        (when (status ?loc joint_fault_exists) ( probabilistic 0.05 ( fault_detected ?loc)))
        (probabilistic 0.15 (and (when (= ?loc left_wing) (and (not (at ?a ?loc))(at ?a about_left_wing)))
