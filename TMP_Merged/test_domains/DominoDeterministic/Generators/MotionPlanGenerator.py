...
class MotionPlanGenerator(Generator):
    def __init__(self, ll_state=None, known_argument_values=None):
        ...
        self.planner = CBiRRTPlanner(timelimit=30.)
    ...
    def goal_param_generator(self):
        ...
        assert generated_val_shape == (4,4) or generated_val_shape == (self.robot.GetActiveDOF(),), "Only 4x4 pose matrix or list of joint " \
            "state values of size self.robot.GetActiveDOF() = "+str(self.robot.GetActiveDOF())+" are acceptable goal values"
        # If goal from generator is a joint configuration list, return list.
        if generated_val_shape == (self.robot.GetActiveDOF(),):
        ...
        # If goal from generator is a pose, find IK solutions for pose.
            ...
            self.goal_configs = self.ll_state.simulator.robots[self.robot_name].get_ik_solutions(self.goal_pose,check_collisions=True)
        else:
    ...
    def planToConfigurations(self,goal_configs):
        ...
        try:
            if len(goal_configs) == 1:
                ...
                trajectory_object = self.planner.PlanToConfiguration(self.robot, goal_configs[0])
            elif len(goal_configs) > 1:
                # If goal is a list of IK solutions, then call PlanToConfigurations
                trajectory_object = self.planner.PlanToConfigurations(self.robot, goal_configs)
            else:
                return None
