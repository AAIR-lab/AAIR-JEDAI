...
class Placed(Predicate):
    ...
    def apply(self, ll_state, generated_values):
        ...
        rel_pose = matrixFromPose([ 0.5, -0.5, -0.5,  0.5, -0.052,0.,0.0265])
        for prop in next_hl_state:
            if "human_placed" in prop:
                if "location1" in prop:
                    central_plank = ll_state.simulator.env.GetKinBody('plank113').GetTransform()
                elif "location2" in prop:
                    central_plank = ll_state.simulator.env.GetKinBody('plank213').GetTransform()
                break
